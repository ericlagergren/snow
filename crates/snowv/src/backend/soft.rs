...
impl State {
    ...
    #[inline]
    pub fn write_keystream_block(&mut self, z: &mut Block) {
        self.keystream(z);
    }

    #[inline]
    pub fn write_keystream_blocks(&mut self, block: &mut [Block]) {
        for block in block {
    ...
    /// ```
    #[inline(always)]
    fn keystream(&mut self, z: &mut Block) {
        let t1 = self.t1();
...
fn aes_ct_enc_round(block1: &[u32; 4], block2: &[u32; 4]) -> ([u32; 4], [u32; 4]) {
    if cfg!(any(
        ...
        ct_aes::aes32::snowv::enc_round(block1, block2)
    } else {
        ct_aes::aes64::snowv::enc_round(block1, block2)
    }
}
